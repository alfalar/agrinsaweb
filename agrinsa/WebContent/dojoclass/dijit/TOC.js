 ...
 * @fileoverview
 *          <p>
 *          A TOC (Table of Contents) widget for ESRI ArcGIS Server JavaScript
 *          API. The namespace is <code>agsjs</code>
 *          </p>
 */
// change log:
// 2012-08-21: fix dojo.fx load that caused IE has to refresh to see TOC.
// 2012-07-26: add ready so it works with compact built (missing dijit._Widget,
// dijit._Templated).
...
// 2011-11-04: v1.06: uniquevalue renderer check on/off using definitions. group
// layer on/off. change css class name. inline style as default. deprecate
// standard style
// 2011-08-11: support for not showing legend or layer list; slider at service
// level config; removed style background.

/* global dojo esri */
// reference:
// http://dojotoolkit.org/reference-guide/quickstart/writingWidgets.html

dojo.provide('dojoclass.dijit.TOC');
...
(function() {
  var link = dojo.create("link", {
    type: "text/css",
    rel: "stylesheet",
    href: dojo.moduleUrl("dojoclass.dijit", "css/TOC.css")
  });
...
dojo.declare("dojoclass.dijit._TOCNode", [dijit._Widget, dijit._Templated], {
  ...
  '<span data-dojo-attach-point="checkContainerNode"></span>' +// <input
																// type="checkbox"
																// data-dojo-attach-point="checkNode"/>'
																// +
  ...
  postCreate: function() {
    ...
    // using the availability of certain property to decide what kind of node to
	// create
    ...
    // console.log(this.legend+","+this.layer+","+this.rootLayer);
    if (this.legend) {
    ...
    if (!this._noCheckNode) {
      ...
      if (dijit.form && dijit.form.CheckBox) {
        chk = new dijit.form.CheckBox({ // looks a bug in dijit. image not
										// renderered until mouse over. bug was
										// closed but still exist.
  ...
  _createRootLayerNode: function(rootLayer) {
	// console.log("******************_createRootLayerNode*****************************");
	// console.log(this.rootLayerTOC);
	// console.log("***********************************************");
  ...
  _createLayerNode: function(layer) {    
    ...
    if (layer._subLayerInfos) {// group layer
      ...
      if (this.rootLayerTOC.info.showGroupCount) {
        // this.labelNode.innerHTML = layer.name + ' (' +
		// layer._subLayerInfos.length + ')'+ " (grupo)";
    	  this.labelNode.innerHTML = layer.name;
      }
      ...
      // 2012-07-21: if setVisibility is called before this widget is built,
		// we want to use the actual visibility instead of the layerInfo.
      //console.log("##################################################################");
      //console.log(layer.name+"  "+layer.id);
      //console.log(this.rootLayer.visibleLayers);      
      ...
      if (layer.renderer && !this.rootLayerTOC.info.noLegend && this.rootLayerTOC.info.mode != 'legend') {
        if (this.rootLayerTOC.toc.style == 'inline' && layer.renderer.symbol) {
          ...
          if (layer.renderer.defaultSymbol) {
            // 2012-02-02: IE7&8 may crash when there is default symbol to be
			// draw with graphics.
            // in this case, transfer the default symbol from legend to
			// renderer.
            rends = [{
              symbol: layer.renderer.defaultSymbol,
              label: layer.renderer.defaultLabel,
              isDefault: true,
              url: layer.renderer.url, // note it is merged from
										// _createRootLayerTOC
              imageData: layer.renderer.imageData,// note it is merged from
													// _createRootLayerTOC
              value: "*"
            }].concat(rends);
          }
  ...
  _createLegendNode: function(rendLeg) {
    // {label:, url: , imageData}
    // here we use a pre-defined rule: if there is a definition expression we
	// will make this layer's unique value on/off
    // note, there is a bug in ArcGIS server (as of 10.01) if the service
	// expession has "OR", the request expression has no effect
    // unless the service expression is enclosed with "()"
    // you can put a bugus "1=1" expression to flag this layer for definition
	// operation.
  ...
  _createChildrenNodes: function(chdn, type) {
    ...
    for (var i = 0, n = chdn.length; i < n; i++) {
      ...
      params.data = chd;
      // console.log(params);
      var node = new dojoclass.dijit._TOCNode(params);
      // console.log(node);
      ...
      if (this.legend) {
        ...
        if (renderer instanceof esri.renderer.UniqueValueRenderer) {
          ...
          dojo.forEach(renderer.infos, function(info) {
            if (info.visible) {
              /*
				 * var seg = '(' + renderer.attributeField; if (info.value) {
				 * seg += '=' + q+info.value+q; } seg += ')'; def.push(seg)
				 */
          ...
          if (def.length == renderer.infos.length) {
            ...
            // even if the layer is checked on, if there is no sub type on, it
			// should not show anything.
            // this.layer._definitionExpression = '1=2';
          } else {
            this.layer.visible = true;
            this.layer._definitionExpression = renderer.attributeField + ' IN (' + def.join(',') + ')';// def.join('
																										// OR
																										// ');
          }
        }
  ...
  _getVisibleLayers: function() {
    ...
    dojo.forEach(this.rootLayer.layerInfos, function(layerInfo) {
      if (layerInfo.subLayerIds) {
		...
		// regardless it's sublayer status
        return;
      } else if (layerInfo.visible) {
        // if (!layerInfo._parentLayerInfo ||
		// layerInfo._parentLayerInfo.visible) {
...
dojo.declare('dojoclass.dijit._RootLayerTOC', [dijit._Widget], {
  _currentIndent: 0,
  rootLayer: null,
  toc: null,
  aplanado:false,
  constructor: function(params, srcNodeRef) {
    ...
    this.toc = params.toc;
    this.aplanado=params.aplanado;
    this.info = params.info || {};    
    //console.log("*********************CREADO _RootLayerTOC****************");    
  ...
  _getLayerInfos: function() {	 
	//console.log("CREANDO NODO PARA "+this.rootLayer.id);
    ...
    if (!this.rootLayer.url || this.info.noLegend || this.info.noLayers) {	
      ...
      // note: renderer info only has simple symbols. If the map layer has
		// complex symbol,
      ...
      if (this.info.mode == 'legend') {   
    	  //console.log("CREANDO LEYENDA PARA:"+this.rootLayer.id);
        this._getLegendInfo();
      } else if (this.info.mode == 'layers') {
    	  //console.log("CREANDO LAYERS PARA:"+this.rootLayer.id);
        this._getLayersInfo();
      } else {
    	  //console.log("CREANDO LEYENDA Y LAYERS PARA:"+this.rootLayer.id);
        this._getLayersInfo();        
      }
  ...
  _createRootLayerTOC: function() {	  
    ...
    if (!layer._tocInfos) {
      ...
      if (layer._legendResponse) {
        ...
        dojo.forEach(layer._legendResponse.layers, function(legInfo) {
          ...
          if (layerInfo && legInfo.legend) {
            ...
            // merge legend symbol to renderers. Ideally the REST API already
			// does that, but
            if (layerInfo.renderer) {
              if (layerInfo.renderer.infos) {
                ...
                // note, merge with renderer, not overwriting symbol so we have
				// choice of using legend or draw symbol.
                ...
                // simple, merge url, imageData, contentType. note merged to
				// renderer, not renderer symbol, avoid loss data.
        	 ...
        	 //console.log(layerInfo);
              if (layerInfo.subLayerIds) {        	  
            	  ...
            	  if(layerInfo.type=="Raster Layer"){
                	  ...
                	  layerInfo.parentLayer=null;
                      subLayerInfos.push(layerLookup[layerInfo.id]);
                      subLayerInfos[subLayerInfos.length-1]._parentLayerInfo = null;
            	  }                          		          	  
              }
          });
          //console.log(subLayerInfos);
          layer.layerInfos = subLayerInfos;
          // END SDP
      }else{
          dojo.forEach(layer.layerInfos, function(layerInfo) {
          if (layerInfo.subLayerIds) {          
      ...
      // finalize the tree structure in _tocInfos, skipping all sublayers
		// because they were nested already.
      var tocInfos = [];
      dojo.forEach(layer.layerInfos, function(layerInfo) {
        if (layerInfo.parentLayerId == -1) {
    ...
    // sometimes IE may fail next step
      this._rootLayerNode = new dojoclass.dijit._TOCNode({
      rootLayerTOC: this,
      rootLayer: layer
    });
  ...
  _onSetVisibleLayers: function(visLayers, doNotRefresh){
    // 2012-07-23:
    // set the actual individual layerInfo's visibility after service's
	// setVisibility call.
	  //console.log("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
...
dojo.declare("dojoclass.dijit.TOC", [dijit._Widget], {
  indentSize: 18,
  swatchSize: [30, 30],
  refreshDelay: 500,
  style: 'inline',
  ...
  /**
	 * @name TOCLayerInfo
	 * @class This is an object literal that specify the options for each map
	 *        rootLayer layer.
	 * @property {esri.layers.ArcGISTiledMapServiceLayer |
	 *           esri.layers.ArcGISDynamicMapServiceLayer} rootLayer: ArcGIS
	 *           Server layer.
	 * @property {string} [title] title. optional. If not specified, rootLayer
	 *           name is used.
	 * @property {Boolean} [slider] whether to show slider for each rootLayer to
	 *           adjust transparency. default is false.
	 * @property {Boolean} [noLegend] whether to skip the legend, and only
	 *           display layers. default is false.
	 * @property {Boolean} [collapsed] whether to collapsed the rootLayer layer
	 *           at beginning. default is false, which means expand if visible,
	 *           collapse if not.
	 * 
	 */
  /**
	 * @name TOCOptions
	 * @class This is an object literal that specify the option to construct a
	 *        {@link TOC}.
	 * @property {esri.Map} [map] the map instance. required.
	 * @property {Object[]} [layerInfos] a subset of layers in the map to show
	 *           in TOC. each object is a {@link TOCLayerInfo}
  ...
  constructor: function(params, srcNodeRef) {
    ...
    if (!this.layerInfos) {          	
      ...
      for (var i = this.map.layerIds.length - 1; i >= 0; i--) {
        ...
        var isMostrable=true;
        if(this.nomostrar){
            for (var j = this.nomostrar.length - 1; j >= 0; j--) {
            	if(this.nomostrar[j]==rootLayer.id){
            		isMostrable=false;
            		break;
            	}
            }        	
        }        
        //console.log(mtitulos);        
        if(isMostrable){
            if (!rootLayer._isBaseMap && !rootLayer._isReference) {
            	var titulo=rootLayer.id;
            	if(this.titulos){
                	var mtitulos = dojo.fromJson(this.titulos);            	
                    for (var k = 0; k<=mtitulos.length-1;k++) {
                    	if(mtitulos[k].OLD_VALUE==rootLayer.id){                		
                    		titulo=mtitulos[k].NEW_VALUE;
                    		//console.log(titulo);
                    		break;
                    	}
                    }                    		
      ...
      var conmapadd=dojo.connect(this.map, 'onUpdateEnd', this, function() {    	  
    	  this.addOrRemove();
      });
  ...
  addOrRemove: function(){
	  this.layerInfos = [];
	  //console.log("----------------             COUNT          -----    >"+this.map.layerIds.length);
      for (var i = this.map.layerIds.length - 1; i >= 0; i--) {
          var rootLayer = this.map.getLayer(this.map.layerIds[i]);
          var isMostrable=true;
          if(this.nomostrar){
  ...
  _createTOC: function() {
	//console.log("EJECUTA CREATETOC");
    dojo.empty(this.domNode);
    this._rootLayerTOCs=new Array();
    //console.log("LAYERINFOS:"+this.layerInfos.length);
    for (var i = 0, c = this.layerInfos.length; i < c; i++) {    	
      // attach a title to rootLayer layer itself
      //console.log("{{{{{{{{{{{{{{{{{{{{}}}}}}}}}}}}}}}}}}}}}}}");
      //console.log(this.layerInfos[i]);
      //console.log("{{{{{{{{{{{{{{{{{{{{}}}}}}}}}}}}}}}}}}}}}}}");
      var rootLayer = this.layerInfos[i].layer;      
      var svcTOC = new dojoclass.dijit._RootLayerTOC({
        rootLayer: rootLayer,
        info: this.layerInfos[i],
        aplanado:this.aplanado,
        toc: this
      });
      this._rootLayerTOCs.push(svcTOC);           
      svcTOC.placeAt(this.domNode);      
    }
    //for(var i=0;i<=this._rootLayerTOCs.length-1;i++){
    //	console.log("LAYERS EN EL TOC:"+this._rootLayerTOCs[i].id);
    //}
